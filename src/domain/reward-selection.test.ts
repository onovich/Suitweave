...
import { createGameState, executeCommand } from './index';

describe('reward offers and reserve', () => {
  it('opens deterministic six-card high rewards and chooses three unique cards', () => {
    const state = { ...createGameState(7), rewards: { ...createGameState(7).rewards, pendingRewards: ['crown' as const] } };
    ...
    for (const option of options.slice(0, 3)) {
      const result = executeCommand(current, { type: 'select-reward-option', optionId: option.id });
  ...
  it('opens three small reward candidates and resolves after one selection', () => {
    ...
    const state = { ...base, rewards: { ...base.rewards, pendingRewards: ['inspiration' as const] } };
    const opened = executeCommand(state, { type: 'open-reward' });
    ...
    const selected = executeCommand(opened.state, { type: 'select-reward-option', optionId: option.id });

    expect(selected).toMatchObject({ ok: true, state: { rewards: { activeOffer: null, pendingRewards: [] } } });
  });
});
