...
import { chooseWildcardInk, discardFeature, endTurn, executeFeatureCard, executePreview, openReward, previewPlacement, selectCard, selectRewardOption, sessionViewModel, setNumberMode, startSession, submitBoard, type GameSession } from '../application';
import type { FeatureTarget } from '../domain';
...
export function App() {
  ...
  const [feature, setFeature] = useState<Readonly<{ cardId: string; targetCount: number; targets: readonly FeatureTarget[] }> | null>(null);
  ...
  const preview = (boardIndex: number, cellIndex: number) => {
    ...
    if (feature !== null) {
      const targets = [...feature.targets, { boardId: board.id, cellId: cell.id }];
      if (targets.length >= feature.targetCount) {
        apply(executeFeatureCard(session, feature.cardId, targets));
        setFeature(null);
  ...
  return <main>
    ...
    <section className="boards">{boards.map((board, boardIndex) => <article className={boardIndex === activeBoard ? 'active-board' : ''} key={board.id}><h2>{board.title}<span>{board.score} 分 · {board.invalidNumberCount} 个警告</span></h2><p className="board-state">{board.locked ? '已锁定' : board.canSubmit ? '可提交' : '继续织入有效组'}</p><div className="grid">{board.cells.map((cell, cellIndex) => <button aria-label={`${board.title} 格子 ${String(cellIndex + 1)} ${cell.label} ${cell.marker ?? ''}`} className={`cell ink-${cell.ink ?? 'none'}`} data-marker={cell.marker ?? undefined} disabled={cell.locked} key={cell.id} onClick={() => { preview(boardIndex, cellIndex); }} onFocus={() => { preview(boardIndex, cellIndex); }}>{cell.label}<span className="marker">{cell.marker === 'crown' ? '♔' : cell.marker === 'inspiration' ? '✦' : ''}</span></button>)}</div>{board.canSubmit && <button className="submit" onClick={() => { apply(submitBoard(session, board.id)); }}>提交并锁定</button>}</article>)}</section>
    ...
    <section className="reserve" aria-label="功能牌备区"><b>功能牌备区 {session.state.rewards.reserve.length} / {session.state.ruleset.functionReserveLimit}</b>{session.state.rewards.reserve.map((card) => <span key={card.id}><button onClick={() => { setFeature({ cardId: card.id, targetCount: featureTargets(card.kind), targets: [] }); }}>使用 {card.kind}</button><button onClick={() => { apply(discardFeature(session, card.id)); }}>弃置</button></span>)}</section>
    {feature !== null && <section className="feature-target" role="status">功能牌目标：还需 {feature.targetCount - feature.targets.length} 格 <button onClick={() => { setFeature(null); }}>取消功能牌</button></section>}
    {session.state.rewards.pendingRewards.length > 0 && session.state.rewards.activeOffer === null && <section className="reward-panel"><b>奖励待处理：{session.state.rewards.pendingRewards[0]}</b><button onClick={() => { apply(openReward(session)); }}>打开奖励候选</button></section>}
    {session.state.rewards.activeOffer !== null && <section className="reward-panel" aria-label="奖励选择"><b>请选择 {session.state.rewards.activeOffer.choicesRemaining} 项奖励</b>{session.state.rewards.activeOffer.options.map((option) => <button key={option.id} onClick={() => { apply(selectRewardOption(session, option.id)); }}>{option.type === 'feature' ? `${option.card.kind}（${option.card.rarity}）` : option.reward}</button>)}</section>}
...
function featureTargets(kind: GameSession['state']['rewards']['reserve'][number]['kind']): number { return kind === 'swap-ink' || kind === 'swap-number' || kind === 'sever-ink' ? 2 : 1; }
