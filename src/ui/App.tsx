...
import { chooseWildcardInk, discardFeature, endTurn, executePreview, openReward, previewPlacement, selectCard, selectRewardOption, sessionViewModel, setNumberMode, startSession, submitBoard, type GameSession } from '../application';
...
export function App() {
  ...
  return <main>
    ...
    <section className="reserve" aria-label="功能牌备区"><b>功能牌备区 {session.state.rewards.reserve.length} / {session.state.ruleset.functionReserveLimit}</b>{session.state.rewards.reserve.map((card) => <button key={card.id} onClick={() => { apply(discardFeature(session, card.id)); }}>{card.kind} · 弃置</button>)}</section>
