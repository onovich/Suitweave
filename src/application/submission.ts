...
export const submitBoard = (
  ...
  if (
    ...
    return reject(
      session,
      "Resolve pending rewards or effects before submitting.",
    );
...
export const settleSession = (session: GameSession): GameSession => ({
  ...
  settlement: calculateSettlement(
    analyzeGame(session.state),
    session.state.ruleset,
    settlementBonuses(session),
  ),
});

function settlementBonuses(session: GameSession) {
  const crownScore = session.state.rewards.crownTriggered
    ? session.state.ruleset.bonuses.crown
    : 0;
  return {
    crownScore,
    markerMeterScore: session.state.rewards.bonusScore - crownScore,
    unusedFunctionScore: Math.min(30, session.state.rewards.reserve.length * 3),
  };
}

function allBoardsLocked(session: GameSession["state"]): boolean {
  return session.boards.every((board) => board.locked);
}
