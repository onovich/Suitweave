...
describe('basic hand and turn state machine', () => {
  ...
  it('discards unused cards, restores three actions, and advances the round', () => {
    ...
    const result = endTurn({ ...session, turn: { ...session.turn, actionsRemaining: 1 } });

    expect(result).toMatchObject({ ok: true });
    if (!result.ok) return;
    expect(result.session.turn).toMatchObject({ round: 2, actionsRemaining: 3, usedCardIds: [] });
  ...
  it('settles at the turn limit and rejects further turn changes', () => {
    ...
    expect(endTurn(settled.session)).toMatchObject({ ok: false, session: settled.session });
  });
});
