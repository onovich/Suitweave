...
export const drawBasicHand = (state: GameState, round: number): DrawnHand => {
  ...
  for (let index = 0; index < state.ruleset.baseCardsDrawn; index += 1) {
    const drawn = drawCard(
      rng,
      `r${String(round)}-c${String(index + 1)}`,
      state.ruleset.inkColors,
    );
    hand.push(drawn.card);
    rng = drawn.rng;
  }
  const bonusInk = state.rewards.bonusInk;
  if (bonusInk !== null)
    hand.push({
      id: `r${String(round)}-bonus-ink`,
      kind: "ink",
      ink: bonusInk,
    });
  return {
    state: {
      ...
      rewards:
        bonusInk === null
          ? state.rewards
          : { ...state.rewards, bonusInk: null },
    },
    hand,
  };
};

export const drawReplacementCard = (
  state: GameState,
  id: string,
): Readonly<{ state: GameState; card: HandCard }> => {
  const drawn = drawCard(state.rng, id, state.ruleset.inkColors);
  return { state: { ...state, rng: drawn.rng }, card: drawn.card };
};

function drawCard(
  rng: SeededRng,
  id: string,
  inks: readonly InkColor[],
): Readonly<{ card: HandCard; rng: SeededRng }> {
...
function drawNumberCard(
  ...
  return {
    card: {
      id,
      kind: "number",
      card: { rank: pick(RANKS, rank.value), suit: pick(SUITS, suit.value) },
      ink: ink.ink,
    },
    rng: ink.rng,
  };
...
function drawInk(
  rng: SeededRng,
  inks: readonly InkColor[],
): Readonly<{ ink: InkColor; rng: SeededRng }> {
...
function pick<Value>(values: readonly Value[], index: number): Value {
  ...
  if (value === undefined)
    throw new RangeError("Random index is outside its collection.");
  return value;
}
